...
public class MyCustomUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        List<SimpleGrantedAuthority> userRoles = roleRepository.findAllById(userRoleRepository.findByUserId(user.getId()).stream()
                .map(UserRole::getRoleId).toList()).stream().map(it-> new SimpleGrantedAuthority(it.getName()))
                .toList();
